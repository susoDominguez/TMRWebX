...
% Extract all recommendations from a given guideline URI
...
% interaction/5 - Filter interactions to only those involving given recommendations
% Determines if interaction elements are all internal (from recommendations) or involve external elements
interaction(Recommendations, Interaction, Label, Elements, External) :-
	 interaction(Interaction, Label, Elements),
	% Check if any element is in the recommendations list
	member(Element, Elements),
	member(Element, Recommendations),
	% Determine if all elements are internal (all in Recommendations)
	(forall(member(E, Elements),
			member(E, Recommendations)) ->
		External = false;
		External = true).
...
server(Port) :-
	http_server(http_dispatch,
		[port(Port), workers(16)]).% Use 16 workers for concurrency
		% Load a specific guideline group into memory
		load_guideline_group(DatasetId, DatasetGraphId)  :- getenv_or_default("FUSEKI_HOST_PORT", "http://localhost:3030/", RawHost),
	...
	catch(rdf_load(MainGuidelinesPath,
			[format('nquads'), register_namespaces(false), base_uri('http://anonymous.org/data/'), graph(DatasetGraphId)]),
		Error,
		(message_to_string(Error, Message),
			format(string(ResponseText),
				'Failed to load guideline ~w (~w): ~w',
				[DatasetId, MainGuidelinesPath, Message]),
			throw(http_reply(not_found(ResponseText))))).

extract_guideline_id(Request, GuidelineId) :-
	(catch(http_parameters(Request,
				[
					guideline_id(Value, [atom])]),
			_,
			fail) ->
	normalize_guideline_id(Value, GuidelineId);
	(memberchk(method(Method),
				Request) ->
	true;
	Method = get),
...
extract_guideline_id_from_body(Request, GuidelineId) :-
	(memberchk(content_type(ContentType),
			Request),
		sub_atom(ContentType, 0, _, _, 'application/json') ->
	http_read_json_dict(Request,
			Dict,
			[json_object(dict)]),
...
normalize_guideline_id(Value, GuidelineId) :-
	...
	normalize_space(string(Trimmed),
		RawString),
	(Trimmed == "" ->
...
fetch_interactions(DatasetId, Interactions) :-
	rdf_transaction((load_guideline_group(DatasetId, _DatasetGraphId),
...
interactions_to_json([interaction(Interaction, Label, Elems, External)|Rest], [_{interaction:Interaction, label:Label, elements:Elems, external:External}|JsonRest]) :-
	 interactions_to_json(Rest, JsonRest).

show_interactions(Request) :-
	(catch((extract_guideline_id(Request, DatasetId),
				fetch_interactions(DatasetId, Interactions),
